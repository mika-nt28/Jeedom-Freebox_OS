...
class FreeboxAPI
{
	...
	public function disques()
	{
		...
		if ($reponse['success']) {
			...
			foreach ($reponse['result'] as $Disques) {
				...
				$commande = $Disque->AddCommand('Occupation [' . $Disques['type'] . '] - ' . $Disques['id'], $Disques['id'], "info", 'numeric', 'Freebox_OS_Disque', '%');
	...
	public function wifiPUT($parametre)
	{
		...
		if ($parametre == 1)
			...
			$return = $this->fetch('/api/v3/wifi/config/', array("enabled" => false), "PUT");
		if ($return === false)
			return false;
		if ($return['success']) {
	...
	public function UpdateSystem()
	{
		try {
			...
			$Commande = $System->AddCommand('Update', 'update', "action", 'other', 'Freebox_OS_System');
