    ...
    public function disk()
    {
        ...
        if ($reponse['success']) {
            ...
            foreach ($reponse['result'] as $disks) {
                ...
                $command = $disk->AddCommand('Occupation du disque - ' . $disks['type'] . ' - (Id ' . $disks['id'] . ')', $disks['id'], 'info', 'numeric', 'default', '%', null, 1, 'default', 'default', 0, 'fas fa-hdd', 0, '0', 100, null, '0', false, false, 'never', true, true);
                $command->event($value);
                log::add('Freebox_OS', 'debug', '└─────────');
            }
        }
